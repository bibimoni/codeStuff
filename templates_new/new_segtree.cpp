...
template <class T, auto F, auto U> struct SegTree {
  static_assert(is_convertible_v<decltype(F), function<T(T, T)>>,
                "F must be T(T, T)");
  static_assert(is_convertible_v<decltype(U), function<T(T, T)>>,
                "U must be T(T, T)");
...
template <class T, T (*F)(T, T), T (*U)(T, T)> struct SegTree {
  ...
  void update(int i, T val) { update(i, i, val, 1, 1, n); }
  ...
  T query(int u, int v) { return query(u, v, 1, 1, n); }
  T query(int i) { return query(i, i, 1, 1, n); }
  ...
  // F usually min function, (g should return true when first enter the
  // function, g(op(st), val) = true) idx_1, return_index, idx_2..., g(value,
  // return_index) = true g(value, return_index + 1) = false
  int max_right(int l, function<bool(T, T)> g, T val) {
    return max_right(1, 1, n, l, g, val);
  }
  int max_right(int id, int l, int r, int left_bound, function<bool(T, T)> g,
                T val) {
  ...
  // F usually max function, (g should return true when first enter the
  // function, g(op(st), val) = true) idx_1, return_index, idx_2..., g(value,
  // return_index) = true g(value, return_index - 1) = false
  int min_left(int r, function<bool(T, T)> g, T val) {
    return min_left(1, 1, n, r, g, val);
  }
  int min_left(int id, int l, int r, int right_bound, function<bool(T, T)> g,
               T val) {
