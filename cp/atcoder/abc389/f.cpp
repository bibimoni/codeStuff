...
signed main() {
  ...
  SegTree<int, 
    [&] (int v_1, int v_2) { return max(v_1, v_2); },
    [&] (int v_1, int v_2) { return v_1 + v_2; }> 
  st_mx(bound, 0, 0); 
  ...
  for (int i = 0; i < n; i++) {
    int to_right = st_mn.max_right(1, [&] (int v_1, int v_2) { return v_1 <= v_2; }, r[i]);
    int to_left = st_mx.min_left(bound, [&] (int v_1, int v_2) { return v_1 >= v_2; }, l[i]);
    if (to_right != -1 && to_left != -1) {
      st_mn.update(to_left, to_right, 1);
      st_mx.update(to_left, to_right, 1);
    }
  }
