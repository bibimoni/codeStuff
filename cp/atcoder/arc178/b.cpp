...
signed main() {
  ...
  while(tt--) {
    ...
    cin >> a[0] >> a[1] >> a[2];
    sort(a.begin(), a.end());
    Mint s1 = power(Mint(10), a[0]) - power(Mint(10), a[0] - 1);
    Mint s2 = power(Mint(10), a[1]) - power(Mint(10), a[1] - 1);
    Mint l, r;
    //a[2] smaller
    if(a[2] == a[1]) {
      r = power(Mint(10), a[2]) - 1;
      l = power(Mint(10), a[0] - 1) + power(Mint(10), a[1] - 1);
    }
    //a[0] + a[1] smaller
    if(a[2] > a[1]) {
      r = power(Mint(10), a[0]) + power(Mint(10), a[1]) - 2;
      l = power(Mint(10), a[2] - 1);
    }
    dbg((int) s1, (int) s2, (int) l, (int) r);
    cout << s1 * s2 * (r - l + 1) << "\n";
  }
}
