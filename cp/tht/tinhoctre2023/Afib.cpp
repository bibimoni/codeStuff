...
int main() {
    ...
    while (tc--) {
        ...
        int fl1 = solve(l), fl2 = solve(l + 1), fr1 = solve(r + 1), fr2 = solve(r + 2);
        int sum1 = (fl1 * (a % mod)) % mod + (fl2 * (a % mod)) % mod;
        int sum2 = (fr1 * (b % mod)) % mod + (fr2 * (b % mod)) % mod;
        int t = (sum2 - sum1 + mod) % mod;
        cout << t << endl;
    }
}