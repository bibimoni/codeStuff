 ...
 */
template <class K, class V> class xMap : public IMap<K, V> {
public:
...
protected:
  ...
  float loadFactor; // define max number of entries can be stored (< (loadFactor
                    // * capacity))

  int (*hashCode)(
      K &, int); // hasCode(K key, int tableSize): tableSize means capacity
  bool (*keyEqual)(K &, K &);   // keyEqual(K& lhs, K& rhs): test if lhs == rhs
  bool (*valueEqual)(V &, V &); // valueEqual(V& lhs, V& rhs): test if lhs ==
                                // rhs
  void (*deleteKeys)(xMap<K, V> *); // deleteKeys(xMap<K,V>* pMap): delete all
                                    // keys stored in pMap
  void (*deleteValues)(xMap<K, V> *); // deleteValues(xMap<K,V>* pMap): delete
...
void xMap<K, V>::initEmptyxMap(int (*hashCode)(K &, int), float loadFactor,
                 bool (*valueEqual)(V &lhs, V &rhs),
                 void (*deleteValues)(xMap<K, V> *),
                 bool (*keyEqual)(K &lhs, K &rhs),
                 void (*deleteKeys)(xMap<K, V> *pMap)) {
  ...
  this->count = 0;
  this->table = new DLinkedList<Entry *>[DEFAULT_CAPACITY];
}
...
xMap<K, V>::xMap(int (*hashCode)(K &, int), float loadFactor,
                 ...
                 void (*deleteKeys)(xMap<K, V> *pMap)) {
  // YOUR CODE IS HERE
  initEmptyxMap(hashCode, loadFactor,
      valueEqual, deleteValues,
      keyEqual, deleteKeys);
}

template <class K, class V> xMap<K, V>::xMap(const xMap<K, V> &map) {
  copyMapFrom(map);
}
...
template <class K, class V> xMap<K, V>::~xMap() {
  // YOUR CODE IS HERE
  removeInternalData();
}
...
V xMap<K, V>::put(K key, V value) {
  ...
  V retValue = value;
  // YOUR CODE IS HERE
  DLinkedList<Entry *> &llist = table[index];
  bool is_key_exist = false;
  for (Entry* item : table[index]) {
    if (keyEQ(item->key, key)) {
      item->value = value;
      is_key_exist = true;
    }
  }
  if (!is_key_exist) {
    Entry *new_item = new Entry(key, value);
    llist.add(new_item);
    count += 1;
...
V xMap<K, V>::remove(K key, void (*deleteKeyInMap)(K)) {
  ...
  for (Entry* item : llist) {
    if (keyEQ(item->key, key)) {
      V retValue = item->value;
      if (deleteKeyInMap != nullptr) {
        deleteKeyInMap(item->key);
      }
      llist.removeItem(item, deleteEntry);
      count -= 1;
      return retValue;
    }
  }
...
bool xMap<K, V>::remove(K key, V value, void (*deleteKeyInMap)(K),
  ...
  DLinkedList<Entry*>& llist = table[index];
  for (Entry* item : llist) {
    if (keyEQ(item->key, key) && valueEQ(item->value, value)) {
      if (deleteKeyInMap != nullptr) {
        deleteKeyInMap(item->key);
      }      
      if (deleteValueInMap != nullptr) {
        deleteValueInMap(item->value);
      }
      llist.removeItem(item, deleteEntry);
      count -= 1;
      return true;
...
template <class K, class V> bool xMap<K, V>::empty() {
  // YOUR CODE IS HERE
  return count == 0;
}

template <class K, class V> int xMap<K, V>::size() {
  // YOUR CODE IS HERE
  return count;
}

template <class K, class V> void xMap<K, V>::clear() {
  // YOUR CODE IS HERE
  removeInternalData();
  initEmptyxMap(this->hashCode, this->loadFactor,
      this->valueEqual, this->deleteValues,
      this->keyEqual, this->deleteKeys);
}

template <class K, class V> DLinkedList<K> xMap<K, V>::keys() {
  // YOUR CODE IS HERE
  DLinkedList<K> returnList;
  for (int index = 0; index < capacity; index++) {
    DLinkedList<Entry*>& llist = table[index];
    for (Entry* item : llist) {
...
template <class K, class V> DLinkedList<V> xMap<K, V>::values() {
  // YOUR CODE IS HERE
  DLinkedList<V> returnList;
  for (int index = 0; index < capacity; index++) {
    DLinkedList<Entry*>& llist = table[index];
    for (Entry* item : llist) {
...
template <class K, class V> DLinkedList<int> xMap<K, V>::clashes() {
  // YOUR CODE IS HERE
  DLinkedList<int> returnList;
  for (int index = 0; index < capacity; index++) {
    returnList.add(table[index].size());
  }
  return returnList;
}
 ...
 */
template <class K, class V> void xMap<K, V>::rehash(int newCapacity) {
 ...
 *      3. Remove table
 */
template <class K, class V> void xMap<K, V>::removeInternalData() {
  // Remove user's data
  if (deleteKeys != 0)
    deleteKeys(this);
  if (deleteValues != 0)
