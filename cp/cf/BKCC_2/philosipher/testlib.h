...
public:
  ...
  unsigned long long next(unsigned long long n) {
    if (n >= (unsigned long long)(__TESTLIB_LONGLONG_MAX))
      __testlib_fail(
          "random_t::next(unsigned long long n): n must be less LONGLONG_MAX");
  ...
  unsigned long long next(unsigned long long from, unsigned long long to) {
    if (from > to)
      __testlib_fail("random_t::next(unsigned long long from, unsigned long "
  ...
  unsigned long next(unsigned long from, unsigned long to) {
    if (from > to)
      __testlib_fail("random_t::next(unsigned long from, unsigned long to): "
  ...
  double next(double from, double to) {
    if (from >= to)
      __testlib_fail("random_t::next(double from, double to): from should be "
  ...
  typename Iter::value_type any(const Iter &begin, const Iter &end) {
    ...
    if (size <= 0)
      __testlib_fail("random_t::any(const Iter& begin, const Iter& end): range "
  ...
  unsigned long long wnext(unsigned long long n, int type) {
    if (n >= (unsigned long long)(__TESTLIB_LONGLONG_MAX))
      __testlib_fail("random_t::wnext(unsigned long long n, int type): n must "
  ...
  unsigned long wnext(unsigned long n, int type) {
    if (n >= (unsigned long)(LONG_MAX))
      __testlib_fail("random_t::wnext(unsigned long n, int type): n must be "
                     "less LONG_MAX");
  ...
  int wnext(int from, int to, int type) {
    if (from > to)
      __testlib_fail("random_t::wnext(int from, int to, int type): from can't "
                     "not exceed to");
  ...
  unsigned long long wnext(unsigned long long from, unsigned long long to,
                           int type) {
    if (from > to)
      __testlib_fail("random_t::wnext(unsigned long long from, unsigned long "
  ...
  long wnext(long from, long to, int type) {
    if (from > to)
      __testlib_fail("random_t::wnext(long from, long to, int type): from "
                     "can't not exceed to");
    return wnext(to - from + 1, type) + from;
  }
  ...
  unsigned long wnext(unsigned long from, unsigned long to, int type) {
    if (from > to)
      __testlib_fail("random_t::wnext(unsigned long from, unsigned long to, "
  ...
  double wnext(double from, double to, int type) {
    if (from >= to)
      __testlib_fail("random_t::wnext(double from, double to, int type): from "
  ...
  typename Container::value_type wany(const Container &c, int type) {
    int size = int(c.size());
    if (size <= 0)
      __testlib_fail("random_t::wany(const Container& c, int type): c.size() "
                     "must be positive");
    typename Container::const_iterator it = c.begin();
  ...
  typename Iter::value_type wany(const Iter &begin, const Iter &end, int type) {
    int size = static_cast<int>(std::distance(begin, end));
    if (size <= 0)
      __testlib_fail("random_t::any(const Iter& begin, const Iter& end, int "
   ...
   * size-1)*/
  template <typename T> std::vector<T> perm(T size) { return perm(size, T(0)); }
  ...
  template <typename T> std::vector<T> partition(int size, T sum, T min_part) {
    ...
    if (result_sum != sum_)
      __testlib_fail(
          "random_t::partition: partition sum is expected to be the given sum");

    if (*std::min_element(result.begin(), result.end()) < min_part)
      __testlib_fail("random_t::partition: partition min is expected to be no "
                     "less than the given min_part");

    if (int(result.size()) != size || result.size() != (size_t)size)
      __testlib_fail("random_t::partition: partition size is expected to be "
...
template <typename C> inline bool isEof(C c) { return c == EOFC; }

template <typename C> inline bool isEoln(C c) { return (c == LF || c == CR); }
  ...
  bool refill() {
    ...
    if (bufferPos >= int(bufferSize)) {
      ...
      if (readSize < BUFFER_SIZE - MAX_UNREAD_COUNT && ferror(file))
        __testlib_fail("BufferedFileInputStreamReader: unable to read (" +
                       getName() + ")");
...
public:
  ...
  void unreadChar(int c) {
    ...
    if (bufferPos < 0)
      __testlib_fail(
          "BufferedFileInputStreamReader::unreadChar(int): bufferPos < 0");
...
struct InStream {
  ...
  readUnsignedLongs(int size, unsigned long long minv, unsigned long long maxv,
                    const std::string &variablesName = "", int indexBase = 1);

  std::vector<unsigned long long> readUnsignedLongs(int size,
                                                    int indexBase = 1);

  unsigned long long readLong(unsigned long long minv, unsigned long long maxv,
                              const std::string &variableName = "");

  std::vector<unsigned long long>
  readLongs(int size, unsigned long long minv, unsigned long long maxv,
            const std::string &variablesName = "", int indexBase = 1);
   ...
   * [minAfterPointDigitCount,maxAfterPointDigitCount] and number is in the form
   * "[-]digit(s)[.digit(s)]".
   */
...
struct ConstantBound {
  ...
  template <typename T> void adjust(T t) {
  ...
  bool has_value() { return !value.empty() && !broken && value.length() < 32; }
};
...
public:
  ...
  std::string testset() const {
    if (!_initialized)
      __testlib_fail(
          "Validator should be initialized with registerValidation(argc, argv) "
  ...
  void adjustConstantBounds(const std::string &variableName, T lower, T upper) {
    if (isVariableNameBoundsAnalyzable(variableName) &&
      ...
      std::string preparedVariableName = prepVariableName(variableName);
      _constantBoundsByVariableName[preparedVariableName].lowerBound.adjust(
          lower);
      _constantBoundsByVariableName[preparedVariableName].upperBound.adjust(
          upper);
    }
  }
  ...
  std::string getConstantBoundsLog() {
    std::string result;
    for (std::map<std::string, ConstantBounds>::iterator i =
             _constantBoundsByVariableName.begin();
         i != _constantBoundsByVariableName.end(); i++) {
      if (i->second.lowerBound.has_value() ||
          i->second.upperBound.has_value()) {
  ...
  void writeTestOverviewLog() {
    if (!_testOverviewLogFileName.empty()) {
      ...
      if (fileName == "stdout")
        ...
        if (NULL == f)
          __testlib_fail("Validator::writeTestOverviewLog: can't write test "
                         "overview log to (" +
                         fileName + ")");
      }
      fprintf(f, "%s%s%s%s", getBoundsHitLog().c_str(),
              getFeaturesLog().c_str(), getConstantBoundsLog().c_str(),
              getVariablesLog().c_str());
      std::fflush(f);
      if (!standard_file)
        if (std::fclose(f))
          __testlib_fail("Validator::writeTestOverviewLog: can't close test "
                         "overview log file (" +
                         fileName + ")");
    }
  }
    ...
    if (!_testMarkupFileName.empty()) {
      ...
      if (!readChars.empty()) {
        ...
        if (_testMarkupFileName == "stdout")
          ...
          if (NULL == f)
            __testlib_fail(
                "Validator::writeTestMarkup: can't write test markup to (" +
                _testMarkupFileName + ")");
        }
        ...
        if (!standard_file)
          if (std::fclose(f))
            __testlib_fail(
                "Validator::writeTestMarkup: can't close test markup file (" +
                _testCaseFileName + ")");
      }
    }
  }
      ...
      if (!readChars.empty()) {
        ...
        if (!testCaseContent.empty()) {
          ...
          if (_testCaseFileName.empty() || _testCaseFileName == "stdout")
            ...
            if (NULL == f)
              __testlib_fail(
                  "Validator::writeTestCase: can't write test case to (" +
                  _testCaseFileName + ")");
          }
          ...
          if (!standard_file)
            if (std::fclose(f))
              __testlib_fail(
                  "Validator::writeTestCase: can't close test case file (" +
                  _testCaseFileName + ")");
        }
      }
    }
...
struct TestlibFinalizeGuard {
  ...
  ~TestlibFinalizeGuard() {
    ...
    if (_alive) {
      ...
      if (!registered)
        __testlib_fail("Call register-function in the first line of the main "
...
std::vector<int> InStream::getReadChars() {
  if (testlibMode != _validator || mode != _input || !stdfile || this != &inf)
    __testlib_fail(
        "InStream::getReadChars can be used only for inf in validator-mode.");
...
NORETURN void InStream::quit(TResult result, const char *msg) {
  ...
  if (__testlib_hasTestCase) {
    if (result != _ok)
      message = __testlib_appendMessage(message, "test case " +
                                                     vtos(__testlib_testCase));
    else {
  ...
  if (mode != _output && result != _fail) {
    if (mode == _input && testlibMode == _validator && lastLine != -1)
      quits(_fail,
            __testlib_appendMessage(__testlib_appendMessage(message, name),
                                    "line " + vtos(lastLine)));
    else
...
char InStream::readChar(char c) {
  ...
  if (c != found) {
    if (!isEoln(found))
      quit(_pe, ("Unexpected character '" + std::string(1, found) + "', but '" +
                 std::string(1, c) + "' expected")
                    .c_str());
    else
...
std::string InStream::readWord(const pattern &p,
  ...
  if (!p.matches(_tmpReadToken)) {
    if (readManyIteration == NO_INDEX) {
      ...
      if (variableName.empty())
        quit(_wa, ("Token element [index=" + vtos(readManyIteration) +
                   "] equals to \"" + __testlib_part(_tmpReadToken) +
                   "\" doesn't correspond to pattern \"" + p.src() + "\"")
                      .c_str());
      else
        quit(_wa,
             ("Token element " + variableName + "[" + vtos(readManyIteration) +
              "] equals to \"" + __testlib_part(_tmpReadToken) +
              "\", doesn't correspond to pattern \"" + p.src() + "\"")
                 .c_str());
    }
...
static inline double stringToDouble(InStream &in, const char *buffer) {
  ...
  for (size_t i = 0; i < length; i++) {
    if (('0' <= buffer[i] && buffer[i] <= '9') || buffer[i] == '.' ||
      ...
      in.quit(_pe,
              ("Expected double, but \"" + __testlib_part(buffer) + "\" found")
                  .c_str());
  }
...
static inline double stringToStrictDouble(InStream &in, const char *buffer,
  ...
  if (minAfterPointDigitCount < 0)
    in.quit(_fail, "stringToStrictDouble: minAfterPointDigitCount should be "
                   "non-negative.");
  ...
  if (afterDigitsCount < minAfterPointDigitCount ||
    ...
    in.quit(
        ...
        ("Expected strict double with number of digits after point in range [" +
         vtos(minAfterPointDigitCount) + "," + vtos(maxAfterPointDigitCount) +
  ...
  if (scanned == 1 || (scanned == 2 && empty)) {
    ...
    if (buffer[0] == '-' && result >= 0)
      in.quit(_pe,
              ("Redundant minus in \"" + __testlib_part(buffer) + "\" found")
                  .c_str());
    return result;
  } else
    in.quit(_pe,
...
static inline long long stringToLongLong(InStream &in, const char *buffer) {
  ...
  if (length == 0 || length > 20)
    in.quit(_pe,
            ("Expected integer, but \"" + __testlib_part(buffer) + "\" found")
                .c_str());
  ...
  if (result < minv || result > maxv) {
    if (readManyIteration == NO_INDEX) {
      ...
      if (variableName.empty())
        ...
        quit(_wa, ("Integer element " + std::string(variableName) + "[" +
...
std::vector<long long> InStream::readLongs(int size, long long minv,
  ...
  __testlib_readMany(readLongs, readLong(minv, maxv, variablesName), long long,
                     true) if (strict && !variablesName.empty())
      validator.addVariable(variablesName);
}
...
unsigned long long InStream::readUnsignedLong(unsigned long long minv,
  ...
  if (result < minv || result > maxv) {
    if (readManyIteration == NO_INDEX) {
      if (variableName.empty())
        quit(_wa, ("Unsigned integer " + vtos(result) +
                   " violates the range [" + toHumanReadableString(minv) +
                   ", " + toHumanReadableString(maxv) + "]")
                      .c_str());
      else
        quit(_wa, ("Unsigned integer parameter [name=" +
                   std::string(variableName) + "] equals to " + vtos(result) +
                   ", violates the range [" + toHumanReadableString(minv) +
                   ", " + toHumanReadableString(maxv) + "]")
                      .c_str());
    } else {
      if (variableName.empty())
        quit(_wa, ("Unsigned integer element [index=" +
                   vtos(readManyIteration) + "] equals to " + vtos(result) +
                   ", violates the range [" + toHumanReadableString(minv) +
                   ", " + toHumanReadableString(maxv) + "]")
                      .c_str());
...
std::vector<unsigned long long>
InStream::readUnsignedLongs(int size, unsigned long long minv,
                            unsigned long long maxv,
                            const std::string &variablesName, int indexBase) {
  __testlib_readMany(
      readUnsignedLongs, readUnsignedLong(minv, maxv, variablesName),
      unsigned long long, true) if (strict && !variablesName.empty())
      validator.addVariable(variablesName);
}

std::vector<unsigned long long> InStream::readUnsignedLongs(int size,
                                                            int indexBase) {
  __testlib_readMany(readUnsignedLongs, readUnsignedLong(), unsigned long long,
                     true)
}
...
double InStream::readReal(double minv, double maxv,
  ...
  if (strict && !variableName.empty()) {
    validator.addBoundsHit(
        variableName, ValidatorBoundsHit(
...
double InStream::readStrictReal(double minv, double maxv,
  ...
  if (result < minv || result > maxv) {
    if (readManyIteration == NO_INDEX) {
      if (variableName.empty())
        quit(_wa, ("Strict double " + vtos(result) + " violates the range [" +
                   vtos(minv) + ", " + vtos(maxv) + "]")
                      .c_str());
      else
        quit(_wa,
             ("Strict double parameter [name=" + std::string(variableName) +
              "] equals to " + vtos(result) + ", violates the range [" +
              vtos(minv) + ", " + vtos(maxv) + "]")
                 .c_str());
    } else {
...
std::vector<double> InStream::readStrictReals(int size, double minv,
  ...
  __testlib_readMany(readStrictReals,
                     ...
                     double, true) if (strict && !variablesName.empty())
      validator.addVariable(variablesName);
}
...
std::vector<double>
InStream::readStrictDoubles(int size, double minv, double maxv,
                            int minAfterPointDigitCount,
                            int maxAfterPointDigitCount,
                            const std::string &variablesName, int indexBase) {
...
void InStream::readStringTo(std::string &result, const pattern &p,
  ...
  if (!p.matches(result)) {
    if (readManyIteration == NO_INDEX) {
      ...
      if (variableName.empty())
        quit(_wa, ("Line element [index=" + vtos(readManyIteration) +
                   "] equals to \"" + __testlib_part(result) +
                   "\" doesn't correspond to pattern \"" + p.src() + "\"")
                      .c_str());
      else
        quit(_wa, ("Line element " + std::string(variableName) + "[" +
...
std::vector<std::string> InStream::readStrings(int size, const pattern &p,
  ...
  __testlib_readMany(readStrings, readString(p, variablesName), std::string,
                     false) if (strict && !variablesName.empty())
      validator.addVariable(variablesName);
}
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp("--testset", argv[i])) {
      if (i + 1 < argc && strlen(argv[i + 1]) > 0)
        ...
        quit(_fail, std::string("Expected non-empty testset after --testset "
    ...
    if (!strcmp("--testCase", argv[i])) {
      if (i + 1 < argc) {
        ...
        if (testCase < 1 || testCase >= __TESTLIB_MAX_TEST_CASE)
          quit(_fail,
               format("Argument testCase should be between 1 and %d, but ",
...
public:
  ...
  std::string testset() const {
    if (!_initialized)
      __testlib_fail(
          "Checker should be initialized with registerTestlibCmd(argc, argv) "
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp("--testset", argv[i])) {
      if (i + 1 < argc && strlen(argv[i + 1]) > 0)
        ...
        quit(_fail,
             std::string(
                 "Expected testset after --testset command line parameter"));
    } else if (!strcmp("--group", argv[i])) {
      if (i + 1 < argc)
...
NORETURN void __testlib_expectedButFound(TResult result, std::string expected,
  ...
  if (strlen(prepend) != 0)
    message =
        format("%s: expected '%s', but found '%s'", compress(prepend).c_str(),
...
template <bool B, class T = void> struct __testlib_enable_if {};

template <class T> struct __testlib_enable_if<true, T> {
  typedef T type;
};
...
template <typename A, typename B> void __println_range(A begin, B end) {
  bool first = true;
...
template <class T, class Enable = void> struct is_iterator {
  static T makeT();
...
template <typename T> T opt(std::false_type is_floating_point, int index);

template <> std::string opt(std::false_type /*is_floating_point*/, int index) {
  return __testlib_indexToArgv(index);
}

template <typename T> T opt(std::true_type /*is_floating_point*/, int index) {
...
bool opt(std::true_type /*is_integral*/, std::true_type /*is_unsigned*/,
  ...
  __testlib_fail("Opts: opt by index '" + toString(index) +
                 "': expected bool true/false or 0/1 but '" + compress(value) +
                 "' found");
}
