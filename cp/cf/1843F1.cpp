...
int n, par[MAX_N][MAX_L], dep[MAX_N];
...
int w, v, u, k, Max[MAX_N] = {0}, Min[MAX_N] = {0}, sufMin[MAX_N] = {0}, sufMax[MAX_N] = {0};
void solve() {
  int q; cin >> q;
  Max[1] = 1; Min[1] = 0; sufMax[1] = 1; sufMin[1] = 0;
  int curr = 2;
  while(q--) {
    char type; cin >> type;
    if(type == '+') {
      cin >> u >> w;
      sufMax[curr] = max(sufMax[u] + w, w);
      sufMin[curr] = min(sufMin[u] + w, w);
      Max[curr] = max(Max[u], sufMax[curr]);
      Min[curr] = min(Min[u], sufMin[curr]);
      curr++; 
    }
    else {
      cin >> u >> v >> k;
      if(Min[v] <= k && k <= Max[v]) YES
      else NO
    }
  }
}
