...
signed main() {
  ...
  if (n <= 10) {
    ...
    auto dfs = [&] (auto &&self, set<int> st, int i) -> int {
      if (i == n) {
        return 1;
      }
      if (memo.count(st)) {
      ...
      int ans = 0;
      for (int j = 1; j <= n; j++) {
        if ((a[j] == 0 || st.find(a[j]) != st.end()) && st.find(j) == st.end()) {
          st.insert(j);
          (ans += self(self, st, i + 1)) %= MOD;
          st.erase(j);
        } 
      }
      return memo[st] = ans;
