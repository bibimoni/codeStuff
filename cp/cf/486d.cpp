...
const int MOD = (int) 1e9 + 7, N = 2e3 + 5;
...
signed main() {
  ...
  Mint ans = 0;
  function<void(int, int, int, vector<Mint>&)> findSet = [&] (int u, int p, int root, vector<Mint> &cnt) {
    cnt[u] = 1;
    for(int v : adj[u]) {
      if(v == p)
        continue;
      if((a[v] < a[root]) || (a[v] > a[root] + d) || (a[root] == a[v] && root > v)) 
        continue;
      findSet(v, u, root, cnt);
      cnt[u] *= (cnt[v] + 1);
    }
  };
