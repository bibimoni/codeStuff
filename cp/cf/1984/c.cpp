...
signed main() {
  ...
  while(tt--) {
    ...
    t2[0] = t2[0] = 0;
    for(int i = 1; i <= n; i++) {
      t1[i] = max({abs(t1[i - 1] + a[i]), t1[i - 1] + a[i], abs(t2[i - 1] + a[i]), t2[i - 1] + a[i]});
      t2[i] = min({abs(t2[i - 1] + a[i]), t2[i - 1] + a[i], abs(t1[i - 1] + a[i]), t1[i - 1] + a[i]});
    }
    vector<Mint> dp1(n + 1, 0);
    vector<Mint> dp2(n + 1, 0);
    dp1[0] = dp2[0] = 1;
    for(int i = 1; i <= n; i++) {
      dp1[i] += dp1[i - 1] * ((t1[i - 1] + a[i] == t1[i]) + (abs(t1[i - 1] + a[i]) == t1[i]));
      dp2[i] += dp2[i - 1] * ((t2[i - 1] + a[i] == t2[i]) + (abs(t2[i - 1] + a[i]) == t2[i]));
      if(dp1[i] != dp2[i]) {
        dp1[i] += dp2[i - 1] * ((t2[i - 1] + a[i] == t1[i]) + (abs(t2[i - 1] + a[i]) == t1[i]));
        dp2[i] += dp1[i - 1] * ((t1[i - 1] + a[i] == t2[i]) + (abs(t1[i - 1] + a[i]) == t2[i]));
      }
    }
    cout << dp1[n] << '\n';
  }
