...
signed main() {
  TLE;
  int meters, numClubs; cin >> meters >> numClubs;
  int clubs[numClubs];
  for(int i = 0; i < numClubs; i++) {
    cin >> clubs[i];
  }
  vector<int> minTrokesAtMeter(meters + 1, 1e9);
  minTrokesAtMeter[0] = 0;
  for(int meter = 1; meter <= meters; meter++) {
    for(int club = 0; club < numClubs; club++) {
      if(clubs[club] <= meter) 
        minTrokesAtMeter[meter] = min(minTrokesAtMeter[meter], minTrokesAtMeter[meter - clubs[club]] + 1);
    }
  }
  if(minTrokesAtMeter[meters] == 1e9) {
    cout << "Roberta acknowledges defeat.";
  }
  else {
    cout << "Roberta wins in " << minTrokesAtMeter[meters] << " strokes.";
  }
}
