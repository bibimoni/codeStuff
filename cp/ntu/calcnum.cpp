...
vector<int> a(31); int n;
vector<vector<vector<bigint>>> dp(31, vector<vector<bigint>>(MAXP, vector<bigint>(MAXN, -1)));
 
// x = a[n - 1]a[n - 2]...a[0]
// digit i, limit with the currnumber, sum of current digit
bigint dfs(int i, bool limit, int sum, int sqrSum) {
  if(i < 0) {
    if(__gcd(sum, sqrSum) == 1) {
      return 1;
    }
    else return 0;
  }
